...
- [x] реализована возможность войти в приложение без авторизации - при отстутствии доступа к MongoDB, для которого может потребоваться подключение VPN (в этом случае данные пользователь получает из приложенного файла с данными). Пользователь сразу попадает на главную страницу
...
- [x] Авторизация (+10)
- [x] Аутентификация (+10)
- [x] Сервер отдаёт корректные ответы, отдаёт HTTP ошибки с нормальными body, по которым можно понять, что произошло, пишет читаемые логи (+20)
